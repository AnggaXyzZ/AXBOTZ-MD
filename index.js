...
async function startXeonBotInc() {
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Subscribe AnggaXyzz','Safari','1.0.0'],
        auth: state
    })
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./angga")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   └───────────────┈ ⳹`
let buttons = [
{buttonId: `wkwwk`, buttonText: {displayText: 'ᴡᴇʟᴄᴏᴍᴇ'}, type: 1}
]
let buttonMessage = {
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   └───────────────┈ ⳹`
let buttons = [
{buttonId: `wkwkwk`, buttonText: {displayText: 'ꜱᴀʏᴏɴᴀʀᴀ'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./Media/theme/cheems.xlsx'),
mimetype: docs,
